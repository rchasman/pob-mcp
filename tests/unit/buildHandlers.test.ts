...
describe('BuildHandlers', () => {
  ...
  describe('handleCompareBuilds', () => {
    ...
    it('should report live DPS instead of N/A', async () => {
      // The DPS row was permanently blank. get_stats now carries damage in its
      // default field set, so what is left to check here is that the handler
      // renders what it is handed rather than dropping it.
      ...
      context.getLuaClient = () => ({
        ...
        getStats: async () => statsFor(call++ === 0 ? 1_500_000 : 900_000),
      }) as any;

      const result = await handleCompareBuilds(context, 'build1.xml', 'build2.xml');

      // the handler formats with toLocaleString(), so build the expectation the
      // same way rather than hardcoding one locale's thousands separator
      expect(result.content[0].text).toContain((1_500_000).toLocaleString());
